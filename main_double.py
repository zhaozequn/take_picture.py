...
import gxipy as gx
from Gui_Double import Ui_Form
import glob
...
model_save_dir = "  modelpath  "

#加载模型
#infer_program, feed_names, fatch_targets = fluid.io.load_inference_model(model_save_dir,infer_exe)
#predict_dict = {}
...
class SpliceGui(Ui_Form, QMainWindow):

    def __init__(self):
        super(SpliceGui, self).__init__()
        self.setupUi(self)
    ...
    def open_frame_apple(self):
        ...
        try:
            ...
            dev_num, dev_info_list = device_manager.update_device_list()
            if dev_num == 0:
                self.statusBar().showMessage("Can Not Find Any Camera")
            ...
            # 初始化拼接图像列
            self.cropped_list1 = np.zeros((260, 1, 3), dtype='uint8')
            self.cropped_list2 = np.zeros((260, 1, 3), dtype='uint8')
            ...
            while start:
                ...
                self.bgr_1 = numpy_image1[..., ::-1]
                self.bgr_2 = numpy_image2[..., ::-1]

                # 校正相机
                self.undistort_img1 = self.img_undistort(self.bgr_1, self.mtx, self.dist)
                self.undistort_img2 = self.img_undistort(self.bgr_2, self.mtx, self.dist)

                # 白平衡后得到待处理图像
                self.img1 = self.white_balance(self.undistort_img1)
                self.img2 = self.white_balance(self.undistort_img2)


                # 显示
                self.show_on_gui(self.img1, self.label_frame1)
                self.show_on_gui(self.img2, self.label_frame1_2)

                # 步骤1，转变色彩通道与调整阈值,形态学处理
                self.img_mask1 = self.adjust(self.img1,self.label_frame2)
                self.img_mask2 = self.adjust(self.img2,self.label_frame2_2)

                # 图像裁剪
                self.img_mask1 = self.img_mask1[140:400, :]
                self.img1 = self.img1[140:400, :]
                self.img_mask2 = self.img_mask2[140:400, :]
                self.img2 = self.img2[140:400, :]


                # 步骤2，轮廓去噪
                self.output1, self.img_roi1, = self.roi_frame(self.img_mask1, self.img1,self.label_frame3,self.label_frame4)
                self.output2, self.img_roi2, = self.roi_frame(self.img_mask2, self.img2,self.label_frame3_2,self.label_frame4_2)

                # 轮廓相关
                # self.fill = self.contours(self.output)
                self.peach1, self.peach_binary1 = self.roi_frame_2(self.output1, self.img_roi1,self.label_frame5)
                self.peach2,self.peach_binary2 = self.roi_frame_2(self.output2,self.img_roi2,self.label_frame5_2)

                if frame_num % 1 == 0:
                    # 得到了完整的轮廓,找轮廓中心
                    self.cropped1 , self.cropped2 = self.findcenter(self.peach1, self.peach_binary1,self.peach2,self.peach_binary2)
                    #self.cropped2 = self.findcenter(self.peach2,self.peach_binary2)

                    # 未抓取到目标
                    if self.cropped1 == [] or self.cropped2 == []:
                    ...
                    # 抓取到目标
                        ...
                        # 计算目标的面积
                        self.area_one = self.area_count(self.peach_binary1)
                        self.area_all.append(self.area_one)

                        self.cropped_list1 = np.append(self.cropped_list1, self.cropped1, axis=1)
                        self.cropped_list2 = np.append(self.cropped_list2, self.cropped2, axis=1)
                        ...
                        if self.cropped_list1.shape[1] >= 600:
                            ...
                            # 取拼接完成后图像去除第一列
                            self.cropped_list1 = self.cropped_list1[:, 1:, :]
                            self.cropped_list2 = self.cropped_list2[:, 1:, :]
                            self.show_on_gui(self.cropped_list1, self.label_frame7)
                            self.show_on_gui(self.cropped_list2, self.label_frame7_2)

                            # 完成横向拼接后进行纵向双相机拼接
                            self.img_cropped = np.append(self.cropped_list1,self.cropped_list2,axis=0)
                            self.show_on_gui(self.img_cropped,self.label_frame9)
                            ...
                            # 对拼接完成的图像进行后续处理
                            #self.yolo_predict(self.img_cropped)
    ...
    def yolo_predict(self,img):
        ...
        """#预测
        predict_result =infer_exe.run(infer_program,
                                      feed={feed_names[0]: predict_img},
                                      fatch_list = fatch_targets)
        """
    ...
    def findcenter(self, img1, binary1,img2,binary2):

        # t = Timer(1,self.findcenter)
        # t.start()
        contours1, hierarchy = cv2.findContours(binary1, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        contours2, hierarchy = cv2.findContours(binary2, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        ...
        if contours1 == [] or contours2 == []:
            ...
            # 用i来遍历轮廓
            for i , j in contours1,contours2:
                print(cv2.contourArea(i))
                print(cv2.contourArea(j))
                if cv2.contourArea(i) > 12000 and cv2.contourArea(j) > 12000:
                    M = cv2.moments(i)
                    N = cv2.moments(j)
                    print('enter findcontour')
                    if M["m00"] and N["m00"] != 0:  # 图像面积为零
                        self.cX1 = int(M["m10"] / M["m00"])  # 找到轮廓中心的X坐标
                        self.cY1 = int(M["m01"] / M["m00"])  # 找到轮廓中心的Y坐标
                        self.cX2 = int(N["m10"] / N["m00"])  # 找到轮廓中心的X坐标
                        self.cY2 = int(N["m01"] / N["m00"])  # 找到轮廓中心的Y坐标
                    # print(self.cX,self.cY)

                    # 判断坐标位置
                    if self.cX1 <= 280:
                        self.cx1_zuo = self.cX1 - 7
                        self.cx1_you = self.cX1 + 6
                        self.cx2_zuo = self.cX2 - 7
                        self.cx2_you = self.cX2 + 6

                    elif 280 < self.cX1 < 360:
                        # 得到左右偏移裁剪量
                        self.cx1_zuo = self.cX1 - 8
                        self.cx1_you = self.cX1 + 8
                        self.cx2_zuo = self.cX2 - 8
                        self.cx2_you = self.cX2 + 8
                    ...
                    # 以轮廓中心为中点提取一定宽度像素矩阵
                    self.cropped1 = img1[:, self.cx1_zuo:self.cx1_you]  # 裁剪坐标为[y0:y1, x0:x1]
                    self.cropped2 = img2[:, self.cx2_zuo:self.cx2_you]

                    self.show_on_gui(self.cropped1, self.label_frame6)
                    self.show_on_gui(self.cropped2, self.label_frame6_2)

                    return self.cropped1, self.cropped2
                else:
                    return [],[]
    ...
    def white_balance(self, img):
        ...
        if self.r_avg != 0 and self.g_avg != 0 and self.b_avg != 0:
    ...
    def adjust(self, img, showGuiWhichLabel):
        ...
        if self.checkBox_adjust.isChecked() == True:
            ...
            if self.checkBox_RGBorHSV.isChecked() == False:
                ...
                # 显示
                self.show_on_gui(self.hsv_mask, showGuiWhichLabel)
                ...
                # 显示
                self.show_on_gui(self.rgb_mask, showGuiWhichLabel)
        ...
        # 这里即为不采用调节窗口
            ...
            if self.checkBox_RGBorHSV.isChecked() == False:
                ...
                # 显示
                self.show_on_gui(self.mask, showGuiWhichLabel)

                return self.mask

    def roi_frame(self, img_in, img_yuantu,showGuiWhichLabelRoi,showGuiWhichLabelOutput):
        ...
        # 显示两部分，分别是roi区域和黑白图
        self.show_on_gui(self.roi, showGuiWhichLabelRoi)
        self.show_on_gui(self.output, showGuiWhichLabelOutput)

        # 返回值中output是黑白二值图，roi是彩图
        return self.output, self.roi

    def roi_frame_2(self, img, roi,showGuiWhichLabel):
        ...
        self.show_on_gui(output, showGuiWhichLabel)
